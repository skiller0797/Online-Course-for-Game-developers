<?php 
//Avadanta Dark css js enqueue

function avadanta_dark_enqueue_scripts()
	{
  ...
  wp_enqueue_style( 'avadanta-dark-style', get_template_directory_uri() . '/style.css',array('bootstrap'));  
	wp_enqueue_style('avadanta-custom', get_stylesheet_directory_uri(). '/assets/css/avadanta-custom.css');
	

	}
	add_action('wp_enqueue_scripts','avadanta_dark_enqueue_scripts');


function avadanta_dark_inline_css(){
...
$avadanta_theme_color_scheme = get_theme_mod('avadanta_theme_color_scheme','linear-gradient( 
90deg, rgb(55 179 135) 0%, rgb(118 191 93) 100%);');               


$avadanta_theme_color_scheme2 = get_theme_mod( 'avadanta_theme_color_scheme2', '#76bf5d' );
...
$avadanta_custom_css      .= '.btn,.btn-theme:hover,.dash::before, .dash::after,
       ...
       {background-image: ' . esc_attr( $avadanta_theme_color_scheme) . ';}';

$avadanta_custom_css      .= '.nav-links .page-numbers,.social li
       {
        background-color: ' . esc_attr( $avadanta_theme_color_scheme2) . ';
        border: 1px solid '. esc_attr( $avadanta_theme_color_scheme2) . '}';
...
$avadanta_custom_css      .= '
       .post-content h4 a:hover,.header-bennar-right ul li a,.wgs-sidebar ul li a:hover,.post-full .post-content h3 a:hover,.reply a,.logged-in-as a,.heading-xs.tc-primary,.copyright a,.nav-links .page-numbers.current,.error-text-large,.post-content-s2 .post-tag,.heading-xs,.tes-author .author-con-s2 .author-name,.readmre a,.srvc .feature-icon,.wgs-sidebar .wgs-heading,.menu .sub-menu li:hover > a, .menu .children li:hover > a, .avadanta-navigate .nav-menu>.page_item>a:hover,.topbar-contact i,.tb-border-design .topbar-socials a,.section-head .heading-xs{
                     color: ' . esc_attr( $avadanta_theme_color_scheme2) . '!important; ;
                }';

$avadanta_custom_css      .= '.avadanta-navigate ul ul
       {
        border-top: 4px solid '. esc_attr( $avadanta_theme_color_scheme2) . '}';


$avadanta_custom_css      .= '.btn-read-more-fill{border-bottom: 1px solid ' . esc_attr( $avadanta_theme_color_scheme2) . ' !important;}';


$avadanta_custom_css      .= ' .nav-links .page-numbers:hover{background-color:  #fff;
                     border-bottom: 1px solid ' . esc_attr( $avadanta_theme_color_scheme2) . ' !important;
                     color:' . esc_attr( $avadanta_theme_color_scheme2) . ' !important;}';
...
$avadanta_custom_css      .= '.contact-banner-area .color-theme, .projects-2-featured-area .featuredContainer .featured-box:hover .overlay,.sidebar-title:before{
    background-color: ' . esc_attr( $avadanta_theme_color_scheme2) . ';opacity:0.8;}';

$avadanta_custom_css      .= '.bg-primary,.slick-dots li.slick-active,.post-full .post-date,.preloader.preloader-dalas:before,
.preloader.preloader-dalas:after,.back-to-top,.wp-block-tag-cloud a:hover{background-color: ' . esc_attr( $avadanta_theme_color_scheme2) . ' !important;}';
...
add_action( 'wp_enqueue_scripts', 'avadanta_dark_inline_css',20 );


  function avadant_dark_custom_header_setup()
    {
        add_theme_support('custom-header', apply_filters('avadanta_custom_header_args', array(
            ...
            'height'             => 250,
            'flex-height'        => true,
            'wp-head-callback'   => 'avadanta_dark_header_style',
        )));
    }

    add_action( 'after_setup_theme', 'avadant_dark_custom_header_setup' );


if ( !function_exists('avadanta_dark_header_style') ) :
    /**
     * Add Header And background Images
     */
    function avadanta_dark_header_style()
    {
...
if (!function_exists('avadanta_custom_excerpt_length')) :
    function avadanta_custom_excerpt_length($length)
    {
        ...
        $avadanta_dark_excerpt_length = get_theme_mod('avadanta_excerpt_length','55');
        if (!empty($avadanta_dark_excerpt_length)) {
            return $avadanta_dark_excerpt_length;
        }
        return 15;
    }
endif;
